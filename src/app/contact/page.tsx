...
import Image from "next/image"; // For grid background
import { motion, AnimatePresence } from "framer-motion"; // For animations
import { Send, CheckCircle, Mail, MapPin, Clock, Terminal, Copy } from "lucide-react"; // Icons, added Copy
...
export default function ContactPage() {
  ...
  const [copiedEmail, setCopiedEmail] = useState(false); // New state for email copy feedback
  const { t } = useLanguage();
  ...
  const handleCopyEmail = () => {
    ...
    setCopiedEmail(true);
    setTimeout(() => setCopiedEmail(false), 2000); // Reset copied status after 2 seconds
  };
  ...
  return (
    <main className="min-h-screen bg-midnight-navy text-white selection:bg-electric-cyan selection:text-midnight-navy">
      ...
      <section className="py-24 bg-[#030B16] relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="lg:sticky lg:top-32">
              ...
              >
                ...
                <h1 className="text-4xl md:text-5xl font-sans font-bold text-white mb-6 text-shadow-glow">
                  {t.contact.title} <br />
                ...
                <p className="text-slate-400 text-lg mb-12 leading-relaxed text-shadow-sm">
                  {t.contact.description}
                </p>

                <div className="space-y-8">
                  <div className="flex items-start gap-4 p-4 rounded-lg transition-all duration-300 border border-electric-cyan/20 hover:bg-white/10 hover:shadow-lg hover:shadow-electric-cyan/10 cursor-pointer">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <p className="text-white font-mono">nexura.officiel@gmail.com</p>
                        <button 
                        ...
                        {copiedEmail && (
                          <span className="text-xs text-electric-cyan animate-fade-in-out">Copied!</span>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            >
              {/* Terminal Window */}
              <div className="bg-midnight-navy border border-electric-cyan/20 rounded-xl overflow-hidden shadow-2xl relative group hover:shadow-electric-cyan/20">
                {/* Corner Accents */}
                ...
                {/* Terminal Header */}
                <div className="bg-white/5 px-4 py-3 border-b border-electric-cyan/20 flex items-center gap-4">
                  <div className="flex gap-2">
                    <div className="w-3 h-3 rounded-full bg-red-500/80"></div>
                    <div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
                    <div className="w-3 h-3 rounded-full bg-green-500/80"></div>
                  </div>
                  <div className="flex-1 text-center font-mono text-xs text-slate-500 flex items-center justify-center gap-2 hover:text-electric-cyan transition-colors group-hover:text-shadow-glow">
                    <Terminal size={12} />
                    root@nexura-uplink:~
                  </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full bg-[#050C16] border border-white/10 rounded px-4 py-3 text-white focus:outline-none focus:border-electric-cyan/50 focus:ring-2 focus:ring-electric-cyan/50 focus:shadow-lg focus:shadow-electric-cyan/20 transition-all placeholder:text-slate-700"
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onBlur={() => setFocusedField(null)}
                          className="w-full bg-[#050C16] border border-white/10 rounded px-4 py-3 text-white focus:outline-none focus:border-electric-cyan/50 focus:ring-2 focus:ring-electric-cyan/50 focus:shadow-lg focus:shadow-electric-cyan/20 transition-all [&>option]:bg-midnight-navy"
                        >
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onBlur={() => setFocusedField(null)}
                          className="w-full bg-[#050C16] border border-white/10 rounded px-4 py-3 text-white focus:outline-none focus:border-electric-cyan/50 focus:ring-2 focus:ring-electric-cyan/50 focus:shadow-lg focus:shadow-electric-cyan/20 transition-all resize-none placeholder:text-slate-700"
